...
async function saveDocument() {
  ...
  const file_extension = filename.substring(filename.lastIndexOf('.') + 1);
  const full_filename = filename.replace(/\.[^/.]+$/, "") + (image_extensions.includes(file_extension) ? ".pdf" :  "." + file_extension);
